...
#=========for tel
	...
	if($loc_line == 0) {$shiftback=0;}

	$shifttel = $rightP1 + $shiftback;
#	$shiftup = $hightx/$loc_line*($loc_line+2);
        $shiftup = $hight/($total_line+3)*($loc_line+2);
#================

	$am1=1e-14*$am; $am2=1e-16*$am;
        $shiftx=-$sepa;
...
#==tel
	...
	$plt00  = " psbasemap  -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -K -P -G255/255/255 -X4i -Y-3i > ${model}_1.ps";
  	$plt1_loc  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M$am1/0 >> $model.ps";
#  	$plt1_loc  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M1e-10/0 >> $model.ps";
  	$plt1_tel  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M$am2/0 >> $model.ps";
#  	$plt1_tel  = "| $pssac -JX${width}i/${hight}i -R$Rw/$Re/$Rs/$Rn  -O -K -Ent-2 -M1e-10/0 >> $model.ps";
...
=cut
	...
	$intdist = int($stna_dist[1]+0.5);
					...
					$pssac_string1[$count_pssac1++]="$nam$com1 $str $pos_y 5/0/255/0\n";      #com1: data:green
 					$pssac_string1[$count_pssac1++]="$nam$com2 $str $pos_y 5/255/0/0\n";      #com2: green: red
					$pstext_string[$count_pstext++]="$str $pos_corr 10 0 0 1 $aa[5*$j+4]\n";  # cross correlation
					$pstext_string[$count_pstext++]="$str $pos_shift 8 0 0 1 $aa[5*$j+6]\n";  # shift second
                                }
                                $offsetx+=$segment[$j];
...
######################################################################################
 open(PLT,$plt200);
  ...
  my @meca_conjugate=`awk -f $ENV{CAPCMDHOME}/to_conjugate.awk $meca[5] $meca[6] $meca[7]`;
