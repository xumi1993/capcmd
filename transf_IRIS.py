...
fl_z = glob.glob("%s/*.??Z.*.SAC" % ddir) # Match each eventsa
stations = [[fl.split('.')[-6], fl.split('.')[-5], fl.split('.')[-4]] for fl in fl_z] # Get station infomation (network, station, location)
os.putenv("SAC_DISPLAY_COPYRIGHT", '0')
p = subprocess.Popen(['sac'], stdin=subprocess.PIPE)
s = '' # Open SAC macro
for sta in stations:
    for comp in glob.glob("%s/*.%s.%s.%s.*.SAC" % (ddir, sta[0], sta[1], sta[2])):
        sacname = os.path.basename(comp) # Get each SAC file in single station
        cname = comp.split('.')[-3] # Get component name
        print(sta,comp,cname)
